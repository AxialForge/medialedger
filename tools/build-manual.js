...
const TOC_ENTRIES = [
  ...
  '21. Security',
  '22. Running MediaLedger on a Raspberry Pi', '22.1 Requirements', '22.2 Install, step by step', '22.3 First run on the Pi', '22.4 Command reference', '22.5 Security on the Pi', '22.6 Moving the desktop database to the Pi', '22.7 File locations', '22.8 Pi troubleshooting',
...
add(H1('21. Security'),
  ...
  ...img('web-security', 'Security tab on the Pi: posture checks, password, options, two-factor codes, sessions and the audit log.'),
  H2('21.1 Always on'),
  bullets([
    'One shared password, hashed with scrypt, stored in `web.json` with owner-only permissions. Minimum 8 characters.',
  ...
  table(['Control', 'What it does'], [
    ['Posture checks', 'Coloured tiles at the top: password set, two-factor, LAN-only, HTTPS, not running as root, secrets file permissions, share credentials permissions, idle sign-out, renaming switched off. Green is good; amber is an optional improvement; red needs attention.'],
    ['Change password', 'Needs the current password. Signs out every other session.'],
  ...
  ...img('web-login', 'Signing in to the web server. The code field is only needed once two-factor is on.'),
);
...
add(H1('22. Running MediaLedger on a Raspberry Pi'),
  ...
  steps([
    'Open `http://medialedger.local:8080` (or `http://<pi-address>:8080`) and sign in with the web password.',
    ...
    'Open **System** during the scan to see temperature and network load, and **Security** afterwards to set up two-factor codes if you want them.',
  ]),
  ...
  P('The service runs as a user with no shell, with `NoNewPrivileges`, `ProtectSystem=full`, `ProtectHome` and a private `/tmp`; it can write only its data folder and the share. The share credentials are root-only in `/etc/medialedger-cifs.cred`; the web password hash, sessions and 2FA secret are owner-only in `/var/lib/medialedger/web.json`.'),
  ...
  table(['Path on the Pi', 'Contents'], [
    ...
    ['`/var/lib/medialedger/web.json`', 'Password hash, sessions, 2FA, options (owner-only)'],
